...
ttr init --remote "$(remote_url mine)" >/dev/null
assert_ok "init with a remote"

# ---- 1. a URL this machine already backs up to is not first contact -------

ttr_answer "y" "y" -- restore --from "$(remote_url mine)" --only config
...
ttr_answer "y" "y" -- restore --from "$(remote_url theirs)" --restart --only config
...
assert_equals "$(sed -n 's/^REMOTE=//p' "$XDG_CONFIG_HOME/tether/config")" "$(remote_url theirs)" \
  "the vault remote follows the vault"

# ---- 3. .git URLs and trailing slashes are the same URL -------------------

ttr set REMOTE="$(remote_url theirs).git" >/dev/null
ttr_answer "y" "y" -- restore --from "$(remote_url theirs)/" --restart --only config
assert_no_output "has not restored from this vault before" "a trailing slash is not a different repo"
ttr set REMOTE="$(remote_url theirs)" >/dev/null

# ---- 4. --allow-unpinned on first contact asks a second time -------------

ttr_answer "y" "n" -- restore --from "$(remote_url mine)" --restart --only plugins --allow-unpinned
...
ttr_answer "y" "y" "y" -- restore --from "$(remote_url mine)" --restart --only plugins --allow-unpinned
assert_ok "accepting the unpinned question proceeds"

# ---- 5. no second question when the vault is one this machine uses -------

ttr set REMOTE="$(remote_url mine)" >/dev/null
ttr_answer "y" "y" -- restore --from "$(remote_url mine)" --restart --only plugins --allow-unpinned
