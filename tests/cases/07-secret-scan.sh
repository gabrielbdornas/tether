...
assert_file "$XDG_STATE_HOME/tether/secrets-found.txt"
assert_no_file "$VAULT/report/secrets-found.txt" "the finding list never enters the vault"
assert_file_lacks "$XDG_STATE_HOME/tether/secrets-found.txt" "ghp_A1b2C3d4" "and holds no copy of the secret"
...
ttr backup -m "should not commit"
...
printf '.local/bin/deploy\n' >>"$XDG_CONFIG_HOME/tether/exclude"
ttr backup -m "clean again"
...
ttr set SECRET_SCAN=warn >/dev/null

# ---- 4. the shapes it knows ------------------------------------------------

rm -f "$XDG_CONFIG_HOME/tether/exclude"
...
ttr set SECRET_SCAN=off >/dev/null
ttr backup -m "scanner off"
assert_ok "backup"
assert_no_output "Possible credentials in the vault" "off does not scan"
ttr set SECRET_SCAN=warn >/dev/null
ttr backup -m "scanner on"
