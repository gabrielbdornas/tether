...
ttr --vault "$VAULT" restore --yes --only config
...
ttr_answer "n" -- --vault "$VAULT" restore --yes --restart --only config
...
ttr_answer "y" -- --vault "$VAULT" restore --yes --restart --only config
...
OUT=""; ttr --vault "$VAULT" restore --yes --restart --only config
...
ttr --vault "$VAULT" restore --yes --restart --only config --enable-units
...
: >"$FAKE_STATE/enabled-units.txt"; : >"$CALLS"
ttr --vault "$VAULT" restore --yes --restart --only config --no-enable-units
assert_not_called "systemctl --user enable" "--no-enable-units leaves them alone"

ttr set ENABLE_UNITS=no >/dev/null
: >"$CALLS"
ttr --vault "$VAULT" restore --yes --restart --only config
...
ttr set ENABLE_UNITS=ask >/dev/null

# ---- 7. ttr enable-units ------------------------------------------------

: >"$FAKE_STATE/enabled-units.txt"; : >"$CALLS"
ttr_answer "n" -- --vault "$VAULT" enable-units
...
ttr --vault "$VAULT" enable-units --all
...
: >"$FAKE_STATE/enabled-units.txt"; : >"$CALLS"
ttr --vault "$VAULT" enable-units --all syncthing.service
assert_called "systemctl --user enable -- syncthing.service"
assert_not_called "systemctl --user enable -- phone-home.service" "only the named unit is enabled"

ttr --vault "$VAULT" enable-units --all totally-made-up.service
...
ttr_tty --vault "$VAULT" restore --yes --restart
...
ttr --vault "$VAULT" enable-units --all nosuch.service
