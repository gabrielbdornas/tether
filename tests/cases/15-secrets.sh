...
assert_file "$XDG_CONFIG_HOME/tether/secrets"

# ---- 1. off by default ----------------------------------------------------

ttr backup -m "no secrets"
assert_ok "backup"
...
printf '.ssh\n' >"$XDG_CONFIG_HOME/tether/secrets"
KEY="$XDG_CONFIG_HOME/tether/secrets.key"
printf 'AGE-SECRET-KEY-TEST\n' >"$KEY"
printf 'age1testtesttesttesttest\n' >"$KEY.pub"
ttr set SECRETS_MODE=recipient SECRETS_RECIPIENT="$KEY.pub" INCLUDE_SECRETS=1
assert_ok "configure recipient mode"

ttr backup -m "with secrets"
...
rm -rf "$HOME/.ssh"
ttr set SECRETS_RECIPIENT=age1exampleexampleexampleexample
ttr restore --yes --restart --only secrets
...
ttr set SECRETS_RECIPIENT="$SANDBOX/gone.key.pub"
ttr restore --yes --restart --only secrets
