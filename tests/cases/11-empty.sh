...
ttr --vault "$VAULT" scan
assert_ok "scan an empty vault"

ttr --vault "$VAULT" enable-units
...
printf '{"schemaVersion":1,"tetherVersion":"1.1.0","createdAt":"2026-01-01T00:00:00Z","machine":{"hostname":"x","user":"y","omarchy":"4","kernel":"6"},"categories":[],"counts":{}}\n' >"$BARE/tether.json"
git -C "$BARE" add -A
git -C "$BARE" -c commit.gpgsign=false commit -q -m bare

ttr --vault "$BARE" restore --yes
...
ttr --vault "$VAULT" restore --yes --restart
...
ttr --vault "$SANDBOX/nowhere" verify
assert_fails "verify without a vault"
assert_output "no vault at"

ttr --vault "$SANDBOX/nowhere" scan
assert_fails "scan without a vault"
assert_output "no vault at"

ttr --vault "$SANDBOX/nowhere" enable-units
assert_fails "enable-units without a vault"
assert_output "no vault at"

ttr --vault "$SANDBOX/nowhere" restore --yes
...
ttr --vault "$A" restore --yes --only packages
assert_ok "restore vault A"
assert_called "pacman -S --needed --noconfirm -- ripgrep"

: >"$CALLS"
ttr --vault "$B" restore --yes --only packages
...
# old snapshot.
jq '.createdAt = "2026-06-06T00:00:00Z"' "$B/tether.json" >"$B/tether.json.new"
mv "$B/tether.json.new" "$B/tether.json"
: >"$CALLS"
ttr --vault "$B" restore --yes --only packages
...
ttr --vault "$A" restore --yes --only packagez
...
ttr --vault "$A" restore --yes --skip pakcages
...
ttr --vault "$A" restore --yes --restart --only packages,config
...
ttr init >/dev/null
ttr set INCLUDE_CONFIG=0 INCLUDE_OMARCHY=0 >/dev/null
ttr backup -m "packages only"
...
ttr backup -m "packages only, blocking"
assert_ok "and block does not stop it either"
assert_no_output "Nothing was committed"
ttr set SECRET_SCAN=warn INCLUDE_CONFIG=1 INCLUDE_OMARCHY=1 >/dev/null
